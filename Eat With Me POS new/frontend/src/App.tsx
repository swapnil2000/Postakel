...
function AppContent() {
	...
	const handleLogin = (userData?: any) => {
		...
		const dynamicUser = userData || {
			...
			shift: 'Current Shift',
			email: `manager@${settings.restaurantName
				.toLowerCase()
				.replace(/\s+/g, '')}.com`,
	...
	const handleNavigate = (screen: string) => {
		...
		if (currentUser && hasModuleAccess(screen)) {
			...
			setRecentlyUsed((prev) => {
				const updated = [screen, ...prev.filter((s) => s !== screen)].slice(
					0,
					4
				);
			...
			addNotification({
				...
				title: 'Access Denied',
				message: `You need ${
					module?.requiredRole || 'higher'
				} permissions to access ${module?.name || 'this module'}`,
			});
		}
	};
		...
		const handleKeyboard = (e: KeyboardEvent) => {
			...
			if (e.altKey && e.key >= '1' && e.key <= '9') {
				...
				const bottomNavItems = bottomNavigation.filter(
					(item) => currentUser && hasModuleAccess(item.id)
				);
	...
	const getContextualHelp = () => {
		const helpTexts: Record<string, string> = {
			dashboard:
				'Overview of your restaurant operations. Quick access to key metrics and modules.',
			pos: 'Process orders, manage tables, and handle payments. Your main billing interface.',
			tables:
				'Manage table status, assignments, and layout. Track occupied and available tables.',
			menu: 'Add, edit, and organize your menu items. Manage categories, prices, and availability.',
			kitchen:
				'Kitchen display for order management. Track cooking status and timing.',
			inventory:
				'Track stock levels, manage suppliers, and monitor ingredient usage.',
			...
			settings:
				'Configure restaurant settings, taxes, currencies, and integrations.',
		};

		return (
			helpTexts[currentModule] ||
			'Navigate through your restaurant management system.'
		);
	};
	...
	return (
		<div className='min-h-screen bg-background flex flex-col'>
			...
			{/* Main Content */}
			<main className='flex-1 overflow-auto pb-20'>{renderScreen()}</main>
				...
				<div
					...
					<div className='absolute bottom-36 right-4 space-y-2 animate-slide-up md:bottom-24'>
						...
						{recentlyUsed.length > 0 && (
							<>
								<div className='text-xs text-white text-center mb-3 font-medium'>
									Recently Used
								</div>
								...
								<>
										...
										<>
											...
											<div className='p-2'>
												{recentlyUsed.slice(0, 4).map((moduleId) => {
													const module = appModules.find(
														(m) => m.id === moduleId
													);
													if (!module || !hasModuleAccess(module.id))
														return null;
													return (
														<button
															key={moduleId}
									...
									<div className='p-2'>
										{appModules
											.filter(
												(m) =>
													hasModuleAccess(m.id) && !recentlyUsed.includes(m.id)
											)
											.map((module) => (
												<button
													key={module.id}
							...
							<div className='space-y-4 mb-6'>
								<div className='flex items-start space-x-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>
											Use the bottom navigation to access main features
										</p>
									</div>
								</div>
								...
								<div className='flex items-start space-x-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>
											Tap the + button for quick access to common tasks
										</p>
									</div>
								</div>
								...
								<div className='flex items-start space-x-3'>
									...
									<div>
										...
										<p className='text-sm text-muted-foreground'>
											Get help anytime with the AI assistant in the header
										</p>
									</div>
								</div>
							</div>
