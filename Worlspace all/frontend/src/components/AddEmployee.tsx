...
export function AddEmployee({ onBack, onSave, organizationData, mode = 'add', employee }: AddEmployeeProps) {
  ...
  const handleSave = () => {
    if (validateStep(currentStep)) {
      const generatedEmployeeId = `EMP${String(Math.floor(Math.random() * 9000) + 1000)}`;
      ...
      const updatedEmployee = {
        ...
        joinDate: formData.startDate || employee?.joinDate,
        location: formData.location,
        manager: formData.manager,
        employeeId: formData.employeeId || employee?.employeeId || generatedEmployeeId,
        lastActive: employee?.lastActive || 'Just now',
        skills: formData.skills.split(',').map(s => s.trim()).filter(s => s),
        performance: employee?.performance ?? 85,
        ...
        birthDate: formData.birthDate || employee?.birthDate,
        employmentType: formData.employmentType || employee?.employmentType || 'full-time',
        workSchedule: formData.workSchedule || employee?.workSchedule || 'standard'
      };
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
            {mode === 'edit' ? <User className="w-8 h-8" /> : <UserPlus className="w-8 h-8" />}
