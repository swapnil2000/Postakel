...
export function TenantProvider({ children }: { children: ReactNode }) {
  ...
  const initializeTenant = async (
    ...
    try {
      ...
      const user = tenantUsers.find(u => u.email === userEmail);
      if (!user && !fromSession) {
      ...
      setCurrentUser(user || tenantUsers[0]);
  ...
  const createTenant = async (
    ...
    try {
      ...
      const tenantId = tenantData.subdomain || `tenant-${Date.now()}`;
      ...
      const adminUser: TenantUser = {
        ...
        permissions: [],
        salary: 0,
        employeeId: `${tenantId.toUpperCase().slice(0, 3)}001`,
        lastLogin: null,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      ...
      mockTenants[tenantId] = newTenant;
      mockUsers[tenantId] = [adminUser];
