...
export function AssetTracker({ 
  ...
  const handleSubmit = () => {
    if (!formData.name.trim() || !formData.details.trim()) {
      toast.error('Please fill in asset name and details');
      return;
    }

    if (formData.locationType === 'employee' && !formData.employeeId) {
      toast.error('Please select an employee');
      return;
    }

    if (formData.locationType === 'company' && !formData.companyLocation) {
  ...
  return (
    <div className="container-mobile py-6 pb-24 space-y-6">
      ...
      <Dialog open={showAddAsset} onOpenChange={(open) => !open && resetForm()}>
            ...
            <div className="space-y-4">
              ...
              {formData.locationType === 'company' && (
                <div>
                  ...
                  <Select value={formData.companyLocation} onValueChange={(value) => setFormData(prev => ({ ...prev, companyLocation: value }))}>
                    <SelectTrigger>
