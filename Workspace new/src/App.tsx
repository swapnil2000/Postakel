...
function AppContent() {
  ...
  useEffect(() => {
    if (tenant && currentUser && getAvailableScreens.length > 0) {
      ...
      const hasEmployeeDetailsAccess = isEmployeeDetailsScreen && hasModuleAccess('team');
      
      // If user doesn't have access to current screen, redirect to dashboard
      if (!currentScreenAccess && !hasChildScreenAccess && !hasEmployeeDetailsAccess && activeScreen !== 'notifications' && activeScreen !== 'more' && activeScreen !== 'dashboard') {
  ...
  const renderScreen = () => {
    ...
    switch (activeScreen) {
      ...
      case 'team':
        return hasModuleAccess('team') ? <TeamDirectory userRole={userRole} onNavigate={handleNavigate} employees={users} onUpdateEmployees={() => {}} /> : <AccessDenied />;
      case 'add-employee':
